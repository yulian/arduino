...
void moveForward() {
  analogWrite(MOTOR_PIN_A, 0);      // 왼쪽 DC 모터를 역방향으로 회전
  analogWrite(MOTOR_PIN_B, 255);
  analogWrite(MOTOR_PIN_C, 255);    // 오른쪽 DC 모터를 정방향으로 회전
  analogWrite(MOTOR_PIN_D, 0);
}

void moveBackward() {
  analogWrite(MOTOR_PIN_A, 255);    // 왼쪽 DC 모터를 정방향으로 회전
  analogWrite(MOTOR_PIN_B, 0);
  analogWrite(MOTOR_PIN_C, 0);      // 오른쪽 DC 모터를 역방향으로 회전
...
void stopMoving() {
  analogWrite(MOTOR_PIN_A, 0);      // 모든 DC 모터의 속도를 0으로 설정
  analogWrite(MOTOR_PIN_B, 0);
